...
class ContentManagService extends cds.ApplicationService {
  async init() {
    ...
    // this.after("NEW", "Folder", async (req) => {
    //   console.log("CORS req length");
    // //   response.set("Access-Control-Allow-Origin", "test");
    // // if (req.res) {
    // //     console.log('CORS Reached 1')
    // //     req.res.set("Access-Control-Allow-Origin", "*");
    // //   }
    //   if (req.length > 0) {
    ...
    //       if (req[i].res) {
    //         console.log("CORS Reached *");
    //         req[i].res.set("Access-Control-Allow-Origin", "*");
    //       }
    //     }
    //   }
    // });
    this.after("*", async (each, req) => {
      ...
      if (req.res) {
        console.log("CORS Reached after");
        req.res.set("Access-Control-Allow-Origin", "*");
        // req.res.set('Access-Control-Allow-Methods: GET, PUT, POST, DELETE, OPTIONS');
      }
    });
    this.before("CREATE", "Folder", async (req) => {
      ...
      // req.res.set("Access-Control-Allow-Origin", "*");
      console.log("CORS Reached Folder");
      const folder = req.data;
      const folder1 = SELECT.from("Folder").where({
        folder_name: folder.folder_name,
        maincategory: folder.maincategory,
      });
      const Items = await cds.tx(req).run(folder1);
      // console.log('testq', Items.length, Items)
      if (Items.length >= 1) {
        req.reject(
          409,
          "Folder Name already exists,Please create with different name"
        );
      }
    ...
    this.before("READ", "Folder", async (req) => {
      ...
      if (req.data.ID != undefined) {
        ...
        const query = UPDATE("Folder")
          .set({ visitedtimes: { "+=": 1 }, lastvisited: lastvisited })
    ...
    this.before("UPDATE", ["Folder", "files"], async (req) => {
      ...
      console.log("CORS Reached Folder1");
      let folderdata = req.data;
      let todaysdate = new Date();
      global.fetch = fetch;
      const unsplash = Unsplash.createApi({
      ...
      if (folderdata.files.length >= 1) {
        for (let i = 0; i < folderdata.files.length; i++) {
          if (
            folderdata.files[i].imageurl == null ||
            folderdata.files[i].imageurl == undefined ||
            folderdata.files[i].imageurl == ""
          ) {
